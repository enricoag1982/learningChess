...
```
World 1─* Lesson 1─* Exercise
  │         │
  │         └─1 Boss (MiniGame)
  ├─1 Assessment (world test / test-out)
  └─ habitat, rank threshold
Concept *─* Exercise, MiniGame     (unit of mastery and review)
Character 1─1 piece type
```

| Entity | Fields |
|---|---|
| World | `id`, `order`, `habitat`, `titleKey`, `lessons[]`, `test` (assessment id) |
| Lesson | `id`, `world`, `order`, `concept`, `character`, `storyKey`, `demo`, `guided[]`, `exercises[]`, `boss` (mini-game id) |
| Concept | `id`, `titleKey` (e.g. `rook-move`, `hanging-piece`, `check-escape`, `mate-in-1`) |
...
| World available | Previous world mastered, or unlocked by test-out / parent |
...
| Rank | Highest rank whose `afterWorld` is mastered |
...
|---|---|---|---|
| Placement | 4 tasks per world, worlds in order | ≥ 3/4 per world | Stops at first failed world; earlier worlds mastered (`placement`) |
...
```
packages/content/
  worlds.yaml                  worlds, habitats, ranks
  characters.yaml
  assessments.yaml
