...
- Done: M0 (`m0`), M1 (`m1`), M2 (`m2`), M3 up to `m3.4`. Pending user actions: Settings → Pages → Source = GitHub Actions; playtests 1–2 (`docs/roadmap.md` §5).
- Next: `m3.5` → `m3`, then M4 iterations `m4.1`–`m4.5` (`docs/roadmap.md` §3.1). Branches `m3-worlds-3-4`, then `m4-rules-play`.
